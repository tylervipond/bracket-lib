...
impl Plugin for BTermBuilder {
    fn build(&self, app: &mut bevy::prelude::App) {
        // MSaa must remain off, for some reason the smoothing of edges leads to strange white lines appearing
        // in the rendered font. This doesn't matter much, we want the font to look pixelated and crispy.
        ...
        app.insert_resource(self.clone())
            .insert_resource(ScreenScaler::new(self.gutter))
            .add_systems(Startup, load_terminals);
        if self.with_diagnostics {
        ...
        app.add_systems(
            PostUpdate,
            (update_consoles, replace_meshes, window_resize).chain(),
        );
